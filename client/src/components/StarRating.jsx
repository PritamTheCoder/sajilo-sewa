export default function StarRating({ rating, size = 'md', showCount, reviewCount }) {
  ...
  return (
    <div className="flex items-center gap-1.5" aria-label={`Rating: ${rating ?? 0} out of 5 stars`}>
      <div className="flex items-center gap-0.5">
        {[1, 2, 3, 4, 5].map((star) => (
          <svg
            key={star}
            className={`${sizeClass} ${star <= filled ? 'text-amber-400' : 'text-slate-200'} transition-colors`}
            fill="currentColor"
            viewBox="0 0 20 20"
            aria-hidden="true"
          >
